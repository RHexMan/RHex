...
holding and drag in any direction.  The plot image will apparently rotate in space.  When you release the button, the image will remain in
...
the ticked segment.  Gnuplot displays only the parts of traces that are contained in the plot box.  Any other plot parts are made
transparent, and are not visible.  When RHex first draws a plot, all the parts of all the traces are contained in the plot box and are
visible.  No rotation will change that situation.
...
physical problem as well as a choice that, in retrospect, is not the right one.  The physical difficulty is that when things are translated,
...
At some point as you zoom in, the square and segment disappear off the canvas, so you can\'t read off item coordinates.  But you can see the
...
center.  Gnuplot does provide translation, but its form is not really the best.  However, it will do, and frequently you will not notice the
difference.

What gnuplot does not do, but what they could have done, is provide a plot box translation.  Which is to say, have a mouse action that
...
gnuplot offers quite a few other manipulations that solve other, special problems.  I briefly mention three:
...
segment.  After such a scaling, angles and trace segment length no longer appear veritical, but, especially for very flat sets of traces,
