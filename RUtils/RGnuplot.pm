#line 1 "RGnuplot.pm"
package RGnuplot;   # Rich's kluge of gnuplot to allow use of local gnuplot executable. System call is in execute(), but there is another in convert().
use strict;
use vars qw($VERSION);
use Carp;
...
sub execute
{
    ...
    # Execute gnuplot
    #my $cmd = qq("$gnuplot" "$self->{_script}");        # KLUGE
        my $cmd = qq("$gnuplot" "$self->{_script}");   # Original
print "cmd=$cmd\n";
