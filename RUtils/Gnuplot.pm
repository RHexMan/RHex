package RUtils::Gnuplot;

## Much to my disappointment, this code is written in a way that makes simple inheritance unworkable.  Thus, I have simply changed the bits of code in the execute function that I needed to.
use strict;
use vars qw($VERSION);
use Carp;
...
use if $^O eq 'MSWin32', 'RUtils::MSWin32';
use if $^O eq 'darwin', 'RUtils::Darwin';
	# These don't cause an error on darwin.

#BEGIN { if ($^O =~ /MSWin32/) {require Scalar::Util; Scalar::Util->import( "looks_like_number" ); } }
#BEGIN { if ($^O =~ /MSWin32/) {require Win32;} }	# But maybe need everything.
#BEGIN { if ($^O =~ /MSWin32/) {require Win32::Process; Win32::Process-> import( "Create" ); } }
#BEGIN { require Win32 if $^O =~ /MSWin/ }
#BEGIN { require Win32::Process if $^O =~ /MSWin/ }

#use if ($^O =~ /MSWin/), Win32::Process;
#use if ($^O =~ /MSWin/), Win32;


# !!! https://stackoverflow.com/questions/46877733/conditional-compilation-in-perl Says that
#	use if $^O eq 'MSWin32', 'Win32';
# works!  Note also the eval option and other discussion.
...
# The following code (and other ideas) for conditional compilation comes from https://stackoverflow.com/questions/46877733/conditional-compilation-in-perl


# Call Gnuplot to generate the image file.  This is the only function that was modified from Chart::Gnuplot.
sub execute
{
    my ($self) = @_;
	
	#print "Entering GnuplotWin32::execute\n";
	...
	if ($^O =~ /MSWin/) {
		...
		$cmd .= " -pause" if ($self->{terminal} =~ /^(ggi|pm|windows|wxt|x11)(\s|$)/);
	} else {
		$cmd = qq("$gnuplot" "$self->{_script}");
...
=begin comment
	...
	if (0) {
		...
		$cmd .= " -pause" if ($self->{terminal} =~ /^(ggi|pm|windows|wxt|x11)(\s|$)/);		
		#print "\nIn execute, before call, gnuplot=$gnuplot\n cmd=$cmd\n";
...
#								DETACHED_PROCESS,
		...
		#print "In execute, after call, processID=$processID\n";	
			# The system call should be doing its own thing now.
	
	} else { # Darwin, changed from Chart::Gnuplot::execute().
		my $cmd = qq("$gnuplot" "$self->{_script}");
		$cmd .= " -" if ($self->{terminal} =~ /^(ggi|pm|windows|wxt|x11)(\s|$)/);
...
#
# Usage example:
# $chart = RUtils::Gnuplot->new(output => "test.png");
# $chart->test;
sub test
{
    my ($self) = @_;
...
# my $data = RUtils::Gnuplot::DataSet->new(...);
# $chart->plot2d($data);
# $chart->convert('gif');
sub convert
{
...
################## RUtils::Gnuplot::DataSet class ##################

package RUtils::Gnuplot::DataSet;
use strict;
use Carp;
use File::Temp qw(tempdir);
...
#
# Usage example:
# my $dataSet = RUtils::Gnuplot::DataSet->new(...);
#
...
sub fit
{
    ...
    $fitted .= $hash{func};
    my $outDS = RUtils::Gnuplot::DataSet->new(
        func   => $fitted,
        params => \%param,
    );
    return($outDS);
...
RUtils::Gnuplot - Plot graph using Gnuplot in Perl on the fly. Just a verbatim copy of Chart::Gnuplot with a modification of the execute() function to draw in a separate process (rather than a thread) in Win32.
...
To plot chart using RUtils::Gnuplot, a chart object and at least one dataset
...
To illustate the features of RUtils::Gnuplot, the best way is to show by
...
When the plotting method (e.g. L<plot2d>) is called, RUtils::Gnuplot would
...
explicitly in L<terminal> of the Chart object, RUtils::Gnuplot would by default
...
may be specified optionally when the object is initiated:
    ...
    my $chart = RUtils::Gnuplot->new(%options);
...
If RUtils::Gnuplot encounters options not mentions above, it would convert them
to Gnuplot set statements. E.g. if the chart object is

    $chart = RUtils::Gnuplot->new(
        ...
        foo => "FOO",
    );
...
This mechanism lets RUtils::Gnuplot support many features not mentioned above
...
Create animated gif. E.g.
    ...
    for (my $i = 0; $i < $T; $i++)
    {
        $c[$i] = RUtils::Gnuplot->new(xlabel => 'x');
        my $ds = RUtils::Gnuplot::DataSet->new(
            func => "sin($i*2*pi/$T + x)",
        );
        $c[$i]->add2d($ds);
    }
...
dataset may be specified optionally when the object is initiated:
    ...
    my $dataset = RUtils::Gnuplot::DataSet->new(%options);
...
=item 5. Chart title, axis label and legend
    ...
    # Data set objects
    my $sine = RUtils::Gnuplot::DataSet->new(
        func  => "sin(x)",
        title => "sine function"
    );
    my $cosine = RUtils::Gnuplot::DataSet->new(
        func  => "cos(x)",
        title => "cosine function"
    );
    my $tangent = RUtils::Gnuplot::DataSet->new(
        func  => "tan(x)",
...
=item 7. Plot a financial time series
    ...
    my $dow = RUtils::Gnuplot::DataSet->new(
        file    => "dj.dat",
...
=item 8. Plot several graphs on the same image
    ...
    my $left = RUtils::Gnuplot->new();
    my $sine = RUtils::Gnuplot::DataSet->new(
        func  => "sin(x)",
    );
    $left->add2d($sine);

    my $center = RUtils::Gnuplot->new();
    my $cosine = RUtils::Gnuplot::DataSet->new(
        func  => "cos(x)",
    );
    $center->add2d($cosine);

    my $right = RUtils::Gnuplot->new();
    my $tangent = RUtils::Gnuplot::DataSet->new(
        func  => "tan(x)",
    );
    $right->add2d($tangent);

    # Place the RUtils::Gnuplot objects in matrix to indicate their locations
    $chart->multiplot([
        [$left, $center, $right]
    ]);
