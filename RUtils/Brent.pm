...
sub brent {
    ...
    #print "\nEntering brent($x1,$x2,$tol,$iters,@_)\n";
    ...
    my $fa = &$func($a,@_);
    my $fb = &$func($b,@_);
    if (any($fa*$fb>0)){die "Error:  Brent - Root must be bracketed.\n"}
    ...
    for (my $iter=0;$iter<$iters;$iter++){
        ...
        if ($keeperInds->nelem){
            # Take the successful indices out of the calculation.  Hold their return values.  Call brent reentrantly on those that are left:
            ...
            if ($continueInds->nelem){
                ...
                my $x0Continue = brent($func,$x1c,$x2c,$tol,$iters,@argsCont);
...
RUtils::Brent - Stable zero-finder using the Brent method.
...
Brent, Richard P. 1973, Algorithms for Minimizations without Derivatives, (Englewood Cliffs, N.J.: Prentice-Hall) Chapters 3 and 4.
